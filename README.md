...
This project is a high-performance, background-running screen capture and video recording automation tool triggered by global hotkeys (Shift+Alt+S), developed for the ASA Intelligence ecosystem.
...
```bash
pip install pillow pyperclip pywin32 mss opencv-python numpy
```

**What do these dependencies do?**
1. **Pillow (PIL)**: Captures images and converts pixel formats.
2. **pywin32**: Provides low-level Windows API access for robust clipboard manipulation and Mutex locks. The global `Shift+Alt+S` shortcut is registered directly through the Windows API (`RegisterHotKey`), which is session-managed by the OS and keeps working reliably after the PC wakes from sleep or the screen is locked.
3. **mss**: An ultra-fast, zero-latency screen capture library used to grab frames for video recording.
4. **opencv-python**: Encodes the captured frames into an MP4 video file.
5. **numpy**: Converts raw screen pixels into OpenCV matrix structures.
6. **pyperclip**: Serves as a fallback system to copy the file path as plain text if the Windows API fails.

### 🕹 Usage
1. Open a terminal in the project directory and run: `python screenshot_tool.py`
2. **Take a Screenshot:** Press `Shift + Alt + S`. Select a region using your mouse.
3. **Record a Video:** Press `Shift + Alt + S`, then press the `R` key. Select a region, and recording will start automatically.
...
Bu proje, ASA Intelligence ekosistemi için tasarlanmış yüksek performanslı, arka planda çalışan ve global kısayollarla (Shift+Alt+S) tetiklenen bir ekran kaydetme (görüntü & video) ve pano (clipboard) otomasyon aracıdır.
...
2. **pywin32**: Windows API erişimi (Pano yönetimi ve Mutex kilitleri) için. `Shift+Alt+S` global kısayolu da doğrudan Windows API'si (`RegisterHotKey`) ile kaydedilir; bu, işletim sistemi tarafından oturum seviyesinde yönetildiği için bilgisayar uykudan uyandığında veya ekran kilitlendiğinde de güvenilir şekilde çalışmaya devam eder.
3. **mss**: Video kaydı için ekranı sıfır gecikmeyle (ultra hızlı) kopyalamak için.
4. **opencv-python**: Kareleri birleştirip MP4 formatında video kodlamak için.
5. **numpy**: Ham pikselleri OpenCV matrisine çevirmek için.
6. **pyperclip**: Pano işlemleri başarısız olursa dosya yolunu düz metin olarak kopyalamak için yedek (fallback) sistem.

### 🕹 Kullanım
1. Terminalde klasör dizinine gidip aracı çalıştırın: `python screenshot_tool.py`
2. **Fotoğraf Çekmek İçin:** Klavyeden `Shift + Alt + S` tuşlarına aynı anda basın. Alanı seçin.
3. **Video Çekmek İçin:** `Shift + Alt + S` yaptıktan sonra `R` tuşuna basın. Alanı seçtiğiniz an kayıt başlar.
